...
import { EmployeesQuery } from '@marshmallow-land/data-access-eployees';
...
export class EmployeeComponent implements OnInit, OnDestroy {
  ...
  ngOnInit(): void {
    ...
    if(this.employeeId) {
      ...
      if(this.employee) {
        ...
        this.getEmployeeSubscription = this.employeesService.getEmployee(this.employeeId).subscribe((data) => {
          this.employee = data;
          this.setEmployee(this.employee);
        });
      }      
  ...
  createEmployee() {
    this.createEmployeeSubscription = this.employeesService.createEmployee(this.newEmployee.value).subscribe(() => {
      this.router.navigate(['/']);
    });
  }

  deleteUser() {
    this.deleteEmployeeSubscription = this.employeesService.deleteEmployee(this.employee.id).subscribe(() => {
      this.router.navigate(['/']);
    });
  }

  updateEmployee() {
    this.updateEmployeeSubscription = this.employeesService.updateEmployee(this.employee.id, this.newEmployee.value).subscribe(() => {
      this.router.navigate(['/']);
    });
  }
